...
- Critical Hits begin at 5% chance, award 10× target Points, and reveal the related repeatable Critical Chance Point upgrade. Critical status is rolled when each target spawns; critical targets stay gold with a sparkling effect until resolved, making the bonus visible before the hit. Repeatable critical-chance levels cost `20 × 1.5^level`, rounded to the nearest integer, add 0.5 percentage points, and cap at 100%.
- A forgiven miss awards no Points, does not advance successful-hit progress, normally resets the consecutive multiplier, relocates the target, reverses direction, and pauses play behind a one-second millisecond countdown before resuming. Shielded Momentum preserves that streak and raises its per-hit growth from 1.05× to 1.07×.
- The first Jackpot reveals a two-column gold Medal shop and awards one Medal. Its fixed order is Golden Gains (1), Larger Targets (1), Point Expansion (1), Shorter Jackpot (2), Golden Safety Net (2), Golden Control (3), Jackpot Mastery (5), and Research (10). Larger Targets and Jackpot Mastery multiply target size independently for a combined 2.25× size; Golden Control multiplies speed scaling by 0.75 and therefore stacks with Steady Hands for a combined 0.6×. The other upgrades add a stacking 2× Point multiplier, up to ten fewer required targets, one additive forgiven miss, and a recorded WIP Research unlock. The desktop layout expands outward for both shops; one-time Point upgrades use three columns while repeatable upgrades auto-fill their row.
- Point Expansion reveals Rapid Recovery (10,000 Points), which halves the effective failure cooldown; Efficient Scaling (25,000), which reduces the growth portion of repeatable cost bases by 25%; and Shielded Momentum (100,000), which preserves streaks through Shield misses and increases consecutive growth from 1.05× to 1.07×.
